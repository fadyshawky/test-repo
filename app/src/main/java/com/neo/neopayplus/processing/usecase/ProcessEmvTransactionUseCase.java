...
public class ProcessEmvTransactionUseCase {
    ...
    public void processOnlineAuthorization(TransactionData transactionData,
        ...
        try {
            ...
            LogUtil.e(TAG, "📋 PIN Type: " + (transactionData.pinType == 0 ? "Offline PIN (already verified by card)"
                    : "Online PIN (needs backend verification)"));

            // Use Field 55 from transaction data (built by Emv55Builder)
            String field55 = transactionData.field55;

            if (field55 == null || field55.isEmpty()) {
                callback.onError(new IllegalStateException("Field 55 (DE55) is missing from transaction data"));
                return;
            }

            LogUtil.e(TAG, "✓ Field 55 (DE55) provided - length: " + (field55.length() / 2) + " bytes");
    ...
    private PaymentApiService.AuthorizationRequest buildAuthorizationRequest(
        ...
        // Always send PIN block if available, regardless of CVM type (online or offline
        // PIN)
        // Backend may need PIN block for audit/logging even if card already verified it
        // offline
        if (transactionData.onlinePinBlock != null &&
            ...
            if (cvmResult.shouldSendPinToBackend) {
                ...
                LogUtil.e(TAG,
                        "✓ Including PIN block in authorization request (offline PIN - card verified, but sending for backend audit)");
            }
            ...
            if (com.neo.neopayplus.BuildConfig.DEBUG) {
                ...
                LogUtil.e(TAG, "  CVM Type: " + cvmResult.description);
            }
        } else {
            if ("42".equals(cvmResult.code)) {
                LogUtil.e(TAG, "⚠️ Offline PIN transaction but no PIN block available - card verified PIN internally");
            } else if ("00".equals(cvmResult.code)) {
                LogUtil.e(TAG, "✓ No PIN required - no PIN block to send");
            } else {
                LogUtil.e(TAG, "⚠️ No PIN block available - transaction may be offline PIN or no PIN required");
            }
        }
